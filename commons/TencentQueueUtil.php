...
class TencentQueueUtil
{
     ...
     * @return ReceiveMessage 返回消息对象模型
     * @throws MqException
     */
    public static function receiveMessage(string $queueName, int $pollingWaitSeconds = 0): ReceiveMessage
    {
     ...
     * @param int $pollingWaitSeconds 本次请求的长轮询等待时间。取值范围0-30秒，默认值0。
     * @return array 返回多个消息对象模型
     * @throws MqException
     */
    public static function batchReceiveMessage(string $queueName, int $numOfMsg, int $pollingWaitSeconds = 0): array
    {
    ...
    private static function receive(string $action, string $queueName, int $numOfMsg = 0, int $pollingWaitSeconds = 0)
    {
        ...
        if (isset($responseJson['code']) && 0 !== $responseJson['code'] && 7000 !== $responseJson['code']) {
